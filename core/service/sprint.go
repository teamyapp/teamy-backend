...
func (s Sprint) copyTaskToSprint(
	...
	if sprint.OwningTeamID != task.OwningTeamID {
		return entity.Task{}, errs.NewError(errs.InvalidArgument, fmt.Sprintf("sprint and task must belong to the same team: sprintID=%v, taskID=%v", toSprintID, newTaskID))
	}
...
func (s Sprint) moveTaskToSprint(
	...
	if len(foundSprintIDs) < 1 {
		return entity.Task{}, errs.NewError(errs.NotFound, fmt.Sprintf("relation not found: sprintID=%v, taskID=%v", fromSprintID, taskID))
	}
...
func (s Sprint) removeTaskFromSprint(ct context.Context, tx *transaction.Transaction, rtTx *realtime.Transaction, sprintID uint64, taskID uint64) (entity.Task, *errs.Error) {
	...
	if len(foundSprintIDs) < 1 {
		return entity.Task{}, errs.NewError(errs.NotFound, fmt.Sprintf("relation not found: sprintID=%v, taskID=%v", sprintID, taskID))
	}
