...
func (i *Attachment) FindAttachmentByIDWithTx(ct context.Context, tx *transaction.Transaction, attachmentID uint64) (entity.Attachment, *errs.Error) {
	...
	if err != nil {
		return entity.Attachment{}, errs.NewError(errs.Unknown, err.Error())
	}
...
func (i *Attachment) DeleteAttachment(ct context.Context, tx *transaction.Transaction, attachmentID uint64) *errs.Error {
	_, err := tx.SQLTx().ExecContext(
		ct,
		`
