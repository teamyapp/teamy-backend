...
func (m Mutation) CreateTaskContextFileUploadSession(
	ct context.Context,
	args struct {
		TaskID graphql.ID
	},
	...
	sessionID, err := m.deps.taskService.CreateTaskContextFileUploadSession(ct, taskID)
	if err != nil {
		m.deps.logger.ErrorWithContext(ct, err)
		return "", errs.ToResolverErr(err)
	}
...
func (m Mutation) FinishTaskContextFileUploadSession(ct context.Context, args struct {
	TaskID              graphql.ID
	...
	attachmentListID, argErr := fromGraphQLIDPtr(args.AttachmentListID)
	if argErr != nil {
		internalErr := errs.NewError(
			errs.InvalidArgument,
			argErr.Error(),
	...
	task, err := m.deps.taskService.FinishTaskContextFileUploadSession(ct, taskID, fileUploadSessionID, attachmentListID)
	if err != nil {
		m.deps.logger.ErrorWithContext(ct, err)
		return Task{}, errs.ToResolverErr(err)
	}
