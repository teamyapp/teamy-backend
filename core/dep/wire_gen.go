...
func InitGraphQLAPI(appName AppMame, serviceName ServiceName, environment env.Environment, logger telemetry.Logger, cloudWebAPIExternalBaseURL CloudWebAPIExternalBaseURL, mapServerURL MapServerURL, cloudAPIClientRegistry *client.Registry, realTimeStateSyncer *realtime.StateSyncer, jwtSigningKey JWTSigningKey, sqlDB *sql.DB) (gql.Service[gql2.Resolver], error) {
	...
	teamMember := sqldb.NewTeamMember(factory)
	teamProjectRelation := sqldb.NewTeamProjectRelation(factory)
	...
	serviceTeam := newTeamService(logger, cloudWebAPIExternalBaseURL, cloudAPIClientRegistry, authorizer, toggles, realTimeStateSyncer, factory, task, sprint, sprintParticipant, team, teamMember, teamProjectRelation, teamFileUploadSession, teamMemberGroup, teamMemberGroupUserRelation, repositoryTeamMemberGroup)
	...
	serviceProject := service.NewProject(logger, cloudAPIClientRegistry, authorizer, toggles, factory, realTimeStateSyncer, project, phase, story, projectPhaseRelation, projectStoryRelation, teamProjectRelation, user, task)
