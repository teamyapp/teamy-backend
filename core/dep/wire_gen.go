...
func InitRealTimeStateSyncer(sdataCollector obs.DataCollector, qlDB *sql.DB) *realtime.StateSyncer {
	...
	user := sqldb.NewUser(sdataCollector, qlDB)
	stateSyncer := realtime.NewStateSyncer(sdataCollector, teamMember, user)
	return stateSyncer
}
...
func InitGraphQLAPI(dataCollector obs.DataCollector, cloudWebAPIExternalBaseURL CloudWebAPIExternalBaseURL, cloudAPIClientRegistry *api.ClientRegistry, realTimeStateSyncer *realtime.StateSyncer, sqlDB *sql.DB) (api2.GraphQL, error) {
	...
	serviceTask := service.NewTask(dataCollector, cloudAPIClientRegistry, task, thread, taskAwaitForRelation, taskSyncer, taskAwaitForRelationSyncer, serviceThread)
	...
	serviceSprint := service.NewSprint(dataCollector, cloudAPIClientRegistry, task, sprint, sprintTaskRelation, taskSyncer, sprintTaskRelationSyncer)
