...
func InitActivityStore(sdataCollector obs.DataCollector) *realtime.ActivityStore {
	activityStore := realtime.NewActivityStore(sdataCollector)
...
func InitRealTimeStateSyncer(sdataCollector obs.DataCollector, activityStore *realtime.ActivityStore, qlDB *sql.DB) *realtime.StateSyncer {
	...
	stateSyncer := realtime.NewStateSyncer(sdataCollector, teamMember, activityStore)
	return stateSyncer
}

func InitGraphQLAPI(dataCollector obs.DataCollector, cloudWebAPIExternalBaseURL CloudWebAPIExternalBaseURL, activityStore *realtime.ActivityStore, cloudAPIClientRegistry *api.ClientRegistry, realTimeStateSyncer *realtime.StateSyncer, sqlDB *sql.DB) (api2.GraphQL, error) {
	...
	serviceTask := service.NewTask(dataCollector, cloudAPIClientRegistry, task, thread, taskAwaitForRelation, sprintParticipant, sprintTaskRelation, taskSyncer, taskAwaitForRelationSyncer, serviceThread, activityStore)
...
func InitTaskRPCAPI(dataCollector obs.DataCollector, cloudAPIClientRegistry *api.ClientRegistry, activityStore *realtime.ActivityStore, realTimeStateSyncer *realtime.StateSyncer, sqlDB *sql.DB) api2.TaskRPC {
	task := sqldb.NewTask(dataCollector, sqlDB)
	thread := sqldb.NewThread(dataCollector, sqlDB)
	taskAwaitForRelation := sqldb.NewTaskAwaitForRelation(dataCollector, sqlDB)
	sprintParticipant := sqldb.NewSprintParticipant(dataCollector, sqlDB)
