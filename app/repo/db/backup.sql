...
INSERT INTO team_member (team_id, user_id) VALUES (3, 9);
...
SELECT task.id, task.goal, task.due_at, task.context, task.owner_user_id, task.work_scope_index, task.effort, task.num_of_unknowns, task.created_at, task.updated_at
FROM team_task
		 INNER JOIN task ON team_task.task_id = task.id
WHERE team_id = 1 AND task_status = 1;

-- Select task need attention
SELECT task.id, task.goal, task.due_at, task.context, task.owner_user_id, task.work_scope_index, task.effort, task.num_of_unknowns, task.created_at, task.updated_at
FROM team_member
		 INNER JOIN task ON team_member.need_attention_task_id = task.id
WHERE team_member.user_id = 1 AND team_member.team_id = 1;

-- Select task delivered by user
SELECT task.id, task.goal, task.due_at, task.context, task.owner_user_id, task.work_scope_index, task.effort, task.num_of_unknowns, task.created_at, task.updated_at
FROM team_task
		 INNER JOIN task ON team_task.task_id = task.id
WHERE team_task.team_id = 1 AND team_task.task_status = 3 AND task.owner_user_id = 1;
